  ...
  \subsection{Modelación dinámica}
         ...
         \newpage
            ...
            \subsubsection{Lagrangiano, energía cinética y energía potencial}
                ...
                A partir de lo anterior, la energía Cinética del robot delta a partir de la simplificación de masas en partículas  \( m_{p} \) , \( ~m_{1} \) , \( ~m_{2} \)  es:
\begin{align*}
...
\end{align*}
                ...
                Reemplazando $\parallel\dot{p}\parallel^{2}=\dot{X}_{p}^{2}+\dot{Y}_{p}^{2}+\dot{Z}_{p}^{2}$
                 \[ T= \left[ \frac{1}{2}m_{P} \left( \dot{X}_{p}^{2}+\dot{Y}_{p}^{2}+\dot{Z}_{p}^{2} \right)  \right] + \left[  \left( \frac{1}{6}m_{1}L_{1}^{2} \right)  \sum _{i=1}^{3} \left( \dot{ \theta }_{i} \right) ^{2} \right] + \left( \frac{m_{2}}{2} \right)  \left[  \sum _{i=1}^{3} \left( \dot{X}_{p}^{2}+\dot{Y}_{p}^{2}+\dot{Z}_{p}^{2} \right) + \left( L_{1}^{2}\dot{ \theta }_{i}^{2} \right)  \right]  \] 
         ...
         \newpage
            ...
            Para obtener los 3 valores de los multiplicadores de Lagrange  \(  \lambda _{l} \)  es necesario establecer 3 ecuaciones de la forma de la ecuación (referencia qliao 0X00), más 3 ecuaciones de restricciones de la forma (referencia lqia 0X00)  \( f_{l} \)  . Además, se tiene en cuenta que el término  \( Q_{j}^{ \left( NU \right) } \left( F_{px},F_{py},F_{pz} \right)  \)  son las fuerzas externas que se aplican al efector final. Por lo tanto, derivando respecto a las coordenadas generalizadas  \( q_{j}= \left[ X_{p},Y_{p},Z_{p} \right]  \)  se obtienen las 3 ecuaciones dinamicas:
            ...
            Se resuelve el siguiente sistema matricial para determinar los multiplicadores a partir de las ecuaciones anteriores:
            ...
            Para la coordenada generalizada  \( q_{j}=X_{p} \) 
        ...
        \begin{align*}
            ...
            Donde  \( F_{x} \)  representa la fuerza externa en dirección x sobre la plataforma móvil.\par
            
            Para la coordenada generalizada  \( q_{j}=Y_{p} \) \par
        \begin{align*}
            ...
            \end{align*} 
             ...
             \[  \sum _{l=1}^{3} \lambda _{l}\frac{ \delta f_{l}^{ \left( h \right) }}{ \delta Y_{p}}= \sum _{l=i=1}^{3} \lambda _{l}\ast 2 \left( Y_{P}-L_{1}\cos  \theta _{i}\sin  \phi _{i}- rsin \phi _{i} \right)  \] \par
            ...
            \end{FlushLeft}\par
             ...
             \[  \left( m_{P}+3m_{2} \right) \ddot{Y}_{p}-2 \sum _{l=i=1}^{3} \lambda _{l} \left( Y_{P}- rsin \phi _{i}-L_{1}\cos  \theta _{i}\sin  \phi _{i} \right) =F_{py} \] \par
            
            
\newpage            Para la coordenada generalizada  \( q_{j}=Z_{p} \) \par
        ...
        \begin{align*}
            ...
            \end{FlushLeft}\par
             ...
             \[  \left[  \left[ m_{P}+3m_{2} \right] \ddot{Z}_{p} \right] - \left[ g \left[ m_{p}+3m_{2} \right]  \right] = \sum _{l=i=1}^{3} \lambda _{l}\ast \left( 2\ast \left( Z_{P}-L_{1}\sin  \theta _{i} \right)  \right) +F_{pz} \] \par
            
             \[  \left[  \left[ m_{P}+3m_{2} \right] \ddot{Z}_{p} \right] - \sum _{l=i=1}^{3} \lambda _{l}\ast \left( 2\ast \left( Z_{P}-L_{1}\sin  \theta _{i} \right)  \right) - \left[ g \left[ m_{p}+3m_{2} \right]  \right] =F_{pz} \] \par
            
             \[  \left[ m_{P}+3m_{2} \right] \ddot{Z}_{p}-2 \sum _{l=i=1}^{3} \lambda _{l} \left( Z_{P}-L_{1}\sin  \theta _{i} \right) - \left[ m_{p}+3m_{2} \right] g=F_{pz} \] \par
            
            
            \vspace{\baselineskip}
            \begin{FlushLeft}
            Finalmente reemplazando en el sistema de ecuaciones: 
            \end{FlushLeft}\par
      ...
      \newpage
       ...
       %%%%%%%%%%%%  Starting New Page here %%%%%%%%%%%%%%            Desde: torque1 torque2 torque3\par
