...
\subsubsection{Dinámica Inversa}
    ...
    El fin de obtener el espacio de trabajo del robot delta es calcular todas las posiciones posibles y viables del efector final. El siguiente diagrama de flujo representa una solución, donde las dimensiones del robot y los parámetros de las restricciones son valores conocidos anteriormente. Es importante recalcar que la nomenclatura en esta sección es la misma que se implementa para determinar el jacobiano del robot delta de la sección \ref{ma_jac}.  
    ...
    Para calcular el espacio de trabajo se crea un nodo en ROS. La tarea especifica de este nodo es calcular y guardar los puntos en el espacio cartesiano del efector final que son viables o posibles según restricciones impuestas. 
    La representación gráfica del nodo se presenta en la ilustración \ref{f:Cap6_ws_1}: 
    
                \begin{figure}[htb]
                \centering
                \includegraphics[width=1\linewidth]{Main/Chapter6/Images6/cap6_ws_1.png}
                \caption{Fotografía de un paraguas}
                \label{f:Cap6_ws_1}
            \end{figure}  
    
    
    En este nodo, las dimensiones del robot y las restricciones son establecidas antes de calcular el espacio de trabajo. La única entrada es el paso \(  \Delta  \theta _{1i} \). La cinematica directa y el jacobiano se calculan a partir del método A, específicamente  el desarrollado en las secciones \ref{ma_cd} y \ref{ma_jac} respectivamente. Se utiliza este método por la razón de que el jacobiano $J$ se puede bipartir en dos matrices $(J_{x}$  y  $J_{ \theta })$ donde cada una de ellas representa una singularidad especifica . Ademas, el metodo A tiene la ventaja que tiene las ecuaciones para determinar los ángulos interiores $\theta_{2i}$ y $\theta_{3i}$.
    ...
    El valor de los parámetros de las restricciones impuestas en este trabajo de grado se presentan y se explican en la tabla \ref{t:cap6_ws_1}: 
            ...
            \hline
             $\Delta\theta _{1i}$ & Pasos de discretizacion de rangos de $\theta_{1i}$ muy bajos implica costo computacional alto& $1^{\circ}$ & $90^{\circ}$ \\
            \hline
            ...
            \hline
             $\theta _{3i}$ & Angulo respecto a la inclinación máxima de las roturas, generalmente son de $13^{\circ}$ & $75^{\circ}$ & $165^{\circ}$ \\
            \hline
             $J_{x}$ & Depende de la precisión o factor de seguridad& $5*10^{-1}$ & $\inf$ \\
            \hline
             $J_{\theta}$ & Depende de la precisión o factor de seguridad & $1*10^{-4}$ & $\inf$ \\
            \hline
        ...
        \endgroup     
                  ...
                  \begin{figure}[htb]
                \centering
                \includegraphics[width=0.2\linewidth]{Main/Chapter6/Images6/cap6_ws_4.png}
                \caption{Fotografía de un paraguas}
                \label{f:Cap6_ws_4}
            \end{figure}    
    ...
    El siguiente algoritmo presenta el pseudocódigo de la función dentro del nodo para determinar el espacio de trabajo:
...
\begin{algorithm}[H]
    ...
    \DontPrintSemicolon
      \KwObjetivo {Encontrar todas las posiciones}
      \Kwfile{$delta.kinematics.Paderborn.tm1.adams$}
      \KwInput{$\theta_1,\theta_2,\theta_3$}
      \KwOutput{$P_0(P_{0x},P_{0y},P_{0z})$}
