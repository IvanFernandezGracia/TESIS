\chapter{Desarrollo de la solución}\label{CAP6}
    ...
    Se sigue una serie de pasos para validad la cinemática y dinamica del robot delta:
        ...
        \end{itemize}
        \item 3. Validación Cinemática y Dinámica
        \begin{itemize}
    ...
    \tikzstyle{process_final} = [rectangle,minimum width=2em, minimum height=1em,text width=12em,text height=1em, text centered, draw=cyan, fill=cyan!30]
    ...
    \begin{tikzpicture}[node distance=2em]
        ...
        \node [process, below of=S1, yshift=-1.9em](S2){Calcular trayectoria en espacio articular \\ $\theta_{i=1,2,3}$ (cinematica inversa)};
        \node [process, below of=S2, yshift=-1.9em](S3){Visualización de trayectoria  a partir de $XYZ$ y \\ $\theta_{i=1,2,3}$ configurando el robot delta en Rviz };
        
        \node (S4) [process, below of=S3, yshift=-1.9em]{Guardar trayectoria  $XYZ$, $\theta_{i=1,2,3}$ \\ y la escala de tiempos $s(t)$ en archivo .txt};
        \node (S5) [process, below of=S4, yshift=-1.9em]{Calcular la velocidad angular  $\dot{\theta}_{i=1,2,3}$ y aceleración angular  $\ddot{\theta}_{i=1,2,3}$ de los actuadores};
        \node (S6) [process, below of=S5, yshift=-1.9em,]{Calcular torque de los motores  $\tau_{i=1,2,3}$ a partir de los datos adquiridos en los pasos anteriores.};
        \node (S7) [process, below of=S6, yshift=-1.9em,]{Guardar torque $\tau_{i=1,2,3}$ en archivo .txt};
        \node (S8) [startend1, below of=S7, yshift=-1.9em]{ADAMS};
        \node (S9) [process, below of=S8, yshift=-1.9em]{Importación de la trayectoria $\theta_{i=1,2,3}$, $XYZ$ y la escala de tiempo $s(t)$ desde ROS};
        \node (S10) [process, below of=S9, yshift=-1.9em,]{Dibujar $XYZ$ en el espacio de simulacion de ADAMS};
        \node (S11) [process, below of=S10, yshift=-1.9em,]{Insertar trayectoria $\theta_{i=1,2,3}$ y escala de tiempo $s(t)$ a los motores o juntas creadas en ADAMS};    
        \node (S12) [process, below of=S11, yshift=-1.9em,]{Guardar Torque $\tau_{i=1,2,3}$ en archivo .txt};
        \node [startend, below of=S12, yshift=-1.9em](S13){Validación \\ Cinemática y Dinámica};
        \node (S14) [process_final, right of=S13, xshift=18em,]{Comparar Torques $\tau_{i=1,2,3}$\\Metodo A\\Metodo B\\ADAMS};
        \draw [line] (S) -- (S1);
        \draw [line] (S1) -- (S2);
        \draw [line] (S2) -- (S3);
        \draw [line] (S3) -- (S4);
...
\end{algorithm}       
    ...
    \subsection{Metodología B}
       ...
       Al igual que la sección anterior, se presentan en las figuras \ref{f:Cap6_funtion_7} a la \ref{f:Cap6_funtion_11} las tareas con respecto a la  metodología B del capitulo \ref{CAP4}. 
    ...
    \begin{figure}[h]
        ...
        \begin{subfigure}
                \centering
                 \begin{tikzpicture}[node distance = 5cm, auto]
                    % Place nodes
                    \node [block1] (funcion) {\textbf{Cinemática Aceleracion\\ Sección (\ref{cap4_mb_subsection_acel})}};
                    \node [block2, left of=funcion] (input) {\textbf{$P_{0x},P_{0y},P_{0z},$\\${\theta }_{1},{\theta }_{2},{\theta }_{3},$\\$\dot{\theta }_{1},\dot{\theta }_{2},\dot{\theta }_{3}$}};
        ...
        \begin{subfigure}
                \centering
                 \begin{tikzpicture}[node distance = 5cm, auto]
                    % Place nodes
                    \node [block1] (funcion) {\textbf{Dinamica \\Inversa\\ Sección (\ref{cap4_mb_subsection_dina})}};
...
\subsubsection{Dinámica Inversa}\label{metodoB_dinamica_inv}
    ...
    \subsection{Espacio de Trabajo}\label{espaciotrabajows1}
     ...
     % Define block styles
\tikzstyle{decision} = [diamond, draw=black!50, fill=black!99,text=white, 
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance = 2cm, auto]
        ...
        \node [block2, below of=theta13,node distance=3cm] (restri) {Posición efector$[P_XP_YP_Z]$ a través de cinemática directa\\$[P_XP_YP_Z]=f(\theta_{11},\theta_{12},\theta_{13})$\\Ángulos interiores $\theta_{2i}$ y $\theta_{3i}$
           \\Jacobiano y sus Determinantes};
        \node [decision, below of=restri] (cumple) {¿Cumple con todas las restricciones?};
...
\end{figure}  
    ...
    La representación gráfica del nodo se presenta en la figura \ref{f:Cap6_ws_1}: 
    ...
    El valor de las restricciones en este trabajo de grado se encuentran en la tabla \ref{t:cap6_ws_1}: 
...
\end{algorithm}
    ...
    \begin{figure}[htb]
        ...
        \centering
        \subfloat[Some figure]{\includegraphics[width=0.49\textwidth]{Main/Chapter6/Images6/DIBUJO531.jpg}}\hfill
        \subfloat[Some other figure]{\includegraphics[width=0.49\textwidth]{Main/Chapter6/Images6/DIBUJO532.jpg}}
        
        \subfloat[Some more]{\includegraphics[width=0.49\textwidth]{Main/Chapter6/Images6/DIBUJO533.jpg}}\hfill
        \subfloat[Some less]{\includegraphics[width=0.49\textwidth]{Main/Chapter6/Images6/DIBUJO534.jpg}}
