 ...
 \end{center}
    ...
    \begin{algorithm}[H]
        ...
        \DontPrintSemicolon
          ...
          \KwInput{Punto inicial$P_i(x,y,z)$, punto final $P_f(x,y,z)$, velocidad maxima $v_{max}$ y aceleración maxima $a_{max}$ (para crear trayectoria)}
          ...
          \SetKwFunction{FSub}{nodo}
           \SetKwProg{Fn}{}{:}{}
            \Fn{\FSub{$P_i(x,y,z),P_f(x,y,z),v_{max} ,a_{max}$}}{
                ...
                \tcc{Metodo B}
                El metodo B tiene exactamente los mismos pasos del metodo A\;
                $\tau_{B,i=1,2,3} =torque.A(XYZ ; \dot{X}\dot{Y}\dot{Z} ; \ddot{X}\ddot{Y}\ddot{Z},{\theta}_{i=1,2,3},\dot{\theta}_{i=1,2,3} \ddot{\theta}_{i=1,2,3})$\;
...
\end{algorithm}       
        ...
        \newpage
         ...
         \begin{algorithm}[H]
                  ...
                  $\dot{J}_{iy}=A'_{iy}+B'_{iy}$\;
                	$A_{0,0}=maxx(phi[0],theta1i[0],punto[0])$\;
                	$A_{0,1}=maxx(phi[1],theta1i[1],punto[0])$\;
                	$A_{0,2}=maxx(phi[2],theta1i[2],punto[0])$\;
                	$A_{1,0}=mayy(phi[0],theta1i[0],punto[1])$\;
                	$A_{1,1}=mayy(phi[1],theta1i[1],punto[1])$\;
                	$A_{1,2}=mayy(phi[2],theta1i[2],punto[1])$\;
                	$A_{2,0}=mazz(theta1i[0],punto[2])$\;
                	$A_{2,1}=mazz(theta1i[1],punto[2])$\;
                	$A_{2,2}=mazz(theta1i[2],punto[2])$\;
                
                	$B_{0,0}=((mnt(m_playload)+(3.0*m2))*((1)*(xp_pp)))-(fpx)$\;
                	$B_{1,0}=((mnt(m_playload)+(3.0*m2))*((1)*(yp_pp)))-(fpy)$\;     
                	$B_{2,0}=((mnt(m_playload)+(3.0*m2))*((1)*(zp_pp-g)))-(fpz)$\;  
