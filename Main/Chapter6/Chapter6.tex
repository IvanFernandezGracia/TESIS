...
\section{ADAMS}
    ...
    El diseño de un prototipo en ADAMS puede ser tan complejo como se quiera, en la figura \ref{f:Cap6_adams_fases_desarrollo} se observa el proceso de desarrollo para un prototipo en ADAMS, en nuestro caso, el modelo busca validar la cinemática y dinámica del robot delta en estudio por lo que trabajaremos solo en las fases de \textbf{construcción} ("build") y \textbf{prueba}. ("test").
    
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.8\linewidth]{Main/Chapter6/Images6/adams/proceso_prototipo_virtual.png}
        \caption{Fases de desarrollo para prototipo virtual REFERENCIA}
        \label{f:Cap6_adams_fases_desarrollo}
    \end{figure}
    
    Un robot delta se compone principalmente de una base fija en la parte superior, una base móvil en la parte inferior y tres cadenas cinemáticas compuestas por un antebrazo unido a la base fija y un brazo unido a la base móvil como se observa en la figura \ref{f:ref1}. Desarrollaremos el modelo desde arriba hacia abajo, basándonos en las medidas del robot especificadas en el capitulo \ref{CAP5}.
    
    
    Siguiendo la introducción de ADAMS presentada en el capitulo \ref{CAP3} abrimos ADAMS y generamos un nuevo modelo con la siguiente configuración:
    ...
    \begin{scope}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item \textbf{X:} 0.0
                \item \textbf{Y:} -9806.65
                \item \textbf{Z:} 0.0
            \end{itemize}
        \end{itemize}
    \end{scope}
    
    Luego de terminar la configuración inicial se debería desplegar el GUI de ADAMS, similar a la figura \ref{f:Cap3-adams_window_GUI}, con la diferencia de que el nombre del modelo sera RobotDelta.
    ...
    % \begin{figure}[H]
    %     \centering
    %     \includesvg[width=1\linewidth]{Main/Chapter6/Images6/adams/proceso_prototipo_virtual.svg}
    %     \caption{Jerarquía de datos en ADAMS}
    %     \label{f:Cap6-adams_proceso_prototipo}
    % \end{figure} 